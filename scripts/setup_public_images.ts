...
// Minimal valid 1x1 JPEG buffer as a placeholder so all image files physically exist in /public/images/
...
for (let i = 0; i < analyzed.length; i++) {
  const p = analyzed[i];
  // Clean product name for filename with Arabic characters and spaces
  const cleanName = (p.name || `product_${p.id}`).replace(/[/\\?%*:|"<>]/g, '_');
  const imageName = `${cleanName}.jpg`;
  p.imageName = imageName;
  // Fallback high-res Unsplash or direct image URL as secondary source if needed, but primary is local file
  // The user requested: src={`/images/${encodeURIComponent(product.imageName)}`}
  p.image = `/images/${encodeURIComponent(imageName)}`;

  // Write physical dummy file in public/images/ with Arabic name and spaces
  const filePath = path.join(publicImagesDir, imageName);
  if (!fs.existsSync(filePath)) {
    fs.writeFileSync(filePath, dummyJpegBuffer);
  }
}
...
for (let i = 0; i < analyzed.length; i++) {
  ...
  const imagePath = imageName ? `/images/\${encodeURIComponent(${JSON.stringify(imageName)})}` : p.image;
  ...
  tsContent += `  {
    ...
    image: \`/images/\${encodeURIComponent(${JSON.stringify(imageName)})}\`,
...
console.log("Successfully created setup script and populated /public/images and products.ts!");
