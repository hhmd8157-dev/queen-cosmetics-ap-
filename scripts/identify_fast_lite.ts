...
console.log(`Found ${items.length} items to analyze with gemini-3.1-flash-lite (fast concurrent).`);
...
Return ONLY valid JSON (no markdown fences) with these exact fields:
{
  ...
  "description": "وصف قصير ومفيد للمنتج بالعربية"
      ...
      if (attempt === 3) {
        console.error(`Failed ID ${item.id} after 3 attempts:`, err?.message || err);
      }
...
async function run() {
  ...
  for (let i = 0; i < items.length; i += concurrency) {
    ...
    console.log(`Processing chunk ${Math.floor(i / concurrency) + 1}/${Math.ceil(items.length / concurrency)} (IDs: ${chunk.map(c => c.id).join(', ')})...`);
  ...
  console.log("All items analyzed and saved to", outputPath);
}
